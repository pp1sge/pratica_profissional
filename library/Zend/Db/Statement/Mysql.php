 <?php
/**
 * Zend Framework MySQL Statement
 *
...
class Zend_Db_Statement_Mysql extends Zend_Db_Statement {
    ...
    public function close() {
        ...
        if ( $this->_sqlresult ) @mysql_free_result( $this->_sqlresult );
        return false;
    }
    ...
    $this->_sqlresult = @mysql_query( $sql_query, $this->_adapter->getConnection() );
    if ( !$this->_sqlresult ) {
        $retval = false;
            require_once 'Zend/Db/Statement/Exception.php';
            throw new Zend_Db_Statement_Exception(
                    'Mysqli statement execute error : ' . mysql_error( $this->_adapter->getConnection() ),
                     mysql_errno( $this->_adapter->getConnection() )
            );
        }
        $this->_columnCount = @mysql_num_fields( $this->_sqlresult );
